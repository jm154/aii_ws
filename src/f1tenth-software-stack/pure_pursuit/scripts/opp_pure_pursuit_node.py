...
class PurePursuit(Node):
    def __init__(self):
        ...
        self.safety_margin_grid = 0   # for grid dilation (collision check)
        self.safety_margin_arc = 0.7     # extra clearance on arcs
        self.obs_search_radius = 0.5
        self.arc_step = 0.10
        self.arc_speed_scale = 1.0
        ...
        self.collision_check_extra = 0.0
        ...
        if data.shape[1] > 5:
            self.ref_speed_array = data[:, 5].astype(float)*0.5
        else:
            # if missing, use default speed
            self.ref_speed_array = np.array([], dtype=float)

        # fallback base speed (m/s)
        self.ref_speed_default = 3.0
        ...
        # ===== Noise filter param (tiny obstacle ignore) =====
        self.declare_parameter('min_avoid_obs_size', 0.1)  # meters
        self.min_avoid_obs_size = self.get_parameter('min_avoid_obs_size').get_parameter_value().double_value
    ...
    def pose_callback(self, msg):
        ...
        # Plan and splice if blocked (with tiny-obstacle ignore)
        planned = False
        on_arc = False
        if coll_idx is not None:
            ...
            # --- tiny obstacle noise filter ---
            C_est, r_est = self._estimate_obstacle_near_local_point(coll_local)
            if r_est is not None and r_est <= float(self.min_avoid_obs_size):
                self.get_logger().info("[COLL] small obstacle -> ignore and keep PP")
            else:
        ...
        if speed_cmd <= 0.5 * max(self.ref_speed_array):
            self.L = 1.0
        else:
            self.L = 2.0
    ...
    def _plan_and_splice_arc_from_collision(self, coll_local, prefer_active=False):
        ...
        # Feasibility check (collision-free on dilated grid)
        feasible = []
        if self._path_collision_free_local_buffered(left_local, out_of_bounds_blocks=False):
            feasible.append(("left", left_local, len(left_local)))
        if self._path_collision_free_local_buffered(right_local, out_of_bounds_blocks=False):
        ...
        if prefer_active:
            ...
            self._splice_arc_into_active_track(idxA, idxB, Cw, Rclr, arc_world)   # [MOD] exact replace inside
            ...
            # splice into ORIGINAL loop copy
            self._splice_arc_into_track(idxA, idxB, Cw, Rclr, arc_world)          # [MOD] exact replace inside
    ...
    def _splice_arc_into_track(self, idxA_orig, idxB_orig, Cw, R, arc_world):
        ...
        # [MOD] exact replacement count: number of removed points (A..B inclusive)
        removed_n = fw_len + 1
        arc_world = self._resample_path_by_count(arc_world, removed_n)  # exact count

        new_track, new_mask = [], []
        inserted_arc = False
        # [MOD] exact replacement: no pre/post bridge injection

        for k in range(N):
            if k == idxA_orig and not inserted_arc:
                # insert arc samples (exact count)
                for p in arc_world:
            ...
            # skip A->B forward span (these are the removed_n points)
            if self._in_forward_span(k, idxA_orig, idxB_orig, N):
                continue

            # keep points outside circle (unchanged)
            w = self.waypoints[k]
    ...
    def _splice_arc_into_active_track(self, idxA_trk, idxB_trk, Cw, R, arc_world):
        ...
        # [MOD] exact replacement count on ACTIVE track
        removed_n = fw_len + 1
        arc_world = self._resample_path_by_count(arc_world, removed_n)  # exact count
    ...
    def _speed_at_track_index(self, track_idx: int) -> float:
        ...
        p = self.track_waypoints[track_idx]
    ...
    def _run_ftg_once(self, scan_msg: LaserScan):
        ...
        if e <= s:
            ...
            speed = 0.5 if np.min(proc) < 0.5 else 0.5
    ...
    # ========================= Resample util (NEW) =========================
    def _resample_path_by_count(self, pts_world: np.ndarray, count: int) -> np.ndarray:
        """[MOD] Resample a polyline to have exactly `count` points (including ends)."""
        if pts_world is None or len(pts_world) == 0:
            return pts_world
        count = max(2, int(count))
        if len(pts_world) == count:
            return pts_world.copy()
        if len(pts_world) == 1:
            return np.repeat(pts_world, count, axis=0)

        segs = np.linalg.norm(np.diff(pts_world, axis=0), axis=1)
        d = np.concatenate([[0.0], np.cumsum(segs)])
        L = d[-1]
        if L <= 1e-9:
            return np.repeat(pts_world[:1], count, axis=0)

        t = np.linspace(0.0, L, count)
        ...
        for i, ti in enumerate(t):
            ...
            if j+1 == len(d):
                ...
                ratio = (ti - d[j]) / max(d[j+1]-d[j], 1e-12)
                out[i] = pts_world[j] * (1.0 - ratio) + pts_world[j+1] * ratio
        return out
